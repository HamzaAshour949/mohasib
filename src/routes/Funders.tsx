...
interface Funder { id: number; code: string; name: string; nameEn: string | null; isActive: number }

export default function FundersPage(): JSX.Element {
  const { t } = useTranslation();
  const qc = useQueryClient();
  const [open, setOpen] = useState(false);
  const [editing, setEditing] = useState<Partial<Funder>>({});

  const { data = [] } = useQuery<Funder[]>({ queryKey: ['funders'], queryFn: () => api.funders.list() as Promise<Funder[]> });

  const save = async (): Promise<void> => {
    const r = await api.funders.save(editing) as { ok: boolean; error?: string };
    if (!r.ok) { alert(r.error ?? t('error')); return; }
    setOpen(false); setEditing({});
    void qc.invalidateQueries({ queryKey: ['funders'] });
  };
  const remove = async (id: number): Promise<void> => {
    if (!confirm(t('confirmDelete'))) return;
    const r = await api.funders.delete(id) as { ok: boolean; error?: string };
    if (!r.ok) { alert(r.error ?? t('error')); return; }
    void qc.invalidateQueries({ queryKey: ['funders'] });
  };

  return (
    <Page title={t('funders')} toolbar={<Btn onClick={() => { setEditing({ isActive: 1 }); setOpen(true); }}>{t('add')}</Btn>}>
      <Table<Funder>
        rows={data}
        cols={[
