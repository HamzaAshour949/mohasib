...
interface Order {
  ...
  partyId: number; partyName: string; warehouseId: number | null; warehouseName: string | null;
...
export default function OrdersPage(): JSX.Element {
  ...
  const { data = [] } = useQuery<Order[]>({ queryKey: ['orders', filter], queryFn: () => api.orders.list(filter || undefined) as Promise<Order[]> });
  ...
  const reset = (): void => {
    ...
    setCurrency(baseCurrency); setCurrencyTouched(false); setDiscMajor('0'); setFeesMajor('0'); setNotes(''); setLines([]);
  };
  ...
  const startConvert = (id: number, defaultWh: number | null): void => {
    setConvertId(id);
    setConvWh(defaultWh ?? warehouses.find(w => w.isDefault)?.id ?? warehouses[0]?.id ?? 0);
    setConvPay('credit');
    ...
    <Page title={t('orders')} toolbar={
      ...
      <Table<Order>
        ...
        cols={[
          ...
          { key: 'actions', header: t('actions'), className: 'w-56 text-end',
            render: r => (r.status === 'open' || r.status === 'partial') ? <>
              <Btn variant="ghost" onClick={() => startConvert(r.id, r.warehouseId)}>{t('convert')}</Btn>{' '}
      ...
      <Modal open={open} dirty={dirty} dirtyLabel={t('unsavedChanges')} onClose={() => { void closeEditor(); }} title={t('new')} wide>
        <div className="grid grid-cols-4 gap-3">
          ...
          <div><Label>Due date</Label><Input type="date" className="ltr-num" value={dueDate} onChange={e => setDueDate(e.target.value)} /></div>
